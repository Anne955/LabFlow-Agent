...
def detect_intent(prompt: str) -> IntentResult:
    ...
    if any(
        term in normalized
        for term in ("完整", "全流程", "跑完整", "full workflow", "run everything", "end to end")
    ):
        return IntentResult(
            intent=FULL_QC_WORKFLOW, confidence=0.9, matched_terms=("full",), language_hint=language
        )
    if "metadata" in normalized and any(
        term in normalized for term in ("只", "only", "inspect", "check", "检查", "看")
    ):
        return IntentResult(
    ...
    if any(term in normalized for term in ("预处理", "归一化", "preprocess", "normalize")):
        return IntentResult(
            intent=PREPROCESS_ONLY,
            confidence=0.9,
            matched_terms=("preprocess",),
            language_hint=language,
        )
    if "qc" in normalized and any(term in normalized for term in ("只", "only")):
        return IntentResult(
            intent=QC_ONLY, confidence=0.9, matched_terms=("qc",), language_hint=language
        )
    for intent, terms in RULES:
        matched = tuple(term for term in terms if term in normalized)
        if matched:
            return IntentResult(
                intent=intent, confidence=0.9, matched_terms=matched, language_hint=language
            )
    return IntentResult(
        intent=FULL_QC_WORKFLOW, confidence=0.55, matched_terms=(), language_hint=language
    )
