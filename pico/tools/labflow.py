...
from .base import ToolResult, relpath
...
def tool_scan_experiment_dir(ctx: ToolContext, args: dict[str, object]) -> ToolResult:
    ...
    if not experiment_dir.exists():
        return ToolResult(
            False,
            f"experiment directory does not exist: {relpath(ctx, experiment_dir)}",
            error_code="not_found",
        )
    if not experiment_dir.is_dir():
        return ToolResult(
            False, f"not a directory: {relpath(ctx, experiment_dir)}", error_code="not_directory"
        )

    metadata_files = sorted(
        [p for p in experiment_dir.iterdir() if p.name.lower() in {"metadata.csv", "metadata.xlsx"}]
    )
    ...
    log_files = sorted(
        [
            ...
            if p.name.lower() in {"instrument_log.txt", "instrument_log.log", "instrument_log.csv"}
        ]
    )
    other_files = sorted(
        [
            p
            for p in experiment_dir.iterdir()
            if p.is_file() and p not in metadata_files and p not in log_files
        ]
    )
    ...
    by_suffix = Counter(
        p.suffix.lower() or "<none>" for p in experiment_dir.rglob("*") if p.is_file()
    )
    metadata = {
        "batch_id": batch_id,
...
def tool_inspect_table(ctx: ToolContext, args: dict[str, object]) -> ToolResult:
    ...
    if path.suffix.lower() != ".csv":
        return ToolResult(
            False,
            f"unsupported table format for {relpath(ctx, path)}; CSV is supported in this build",
    ...
    if "sample_id" in columns:
        counts = Counter(
            row.get("sample_id", "") for row in rows if not _is_blank(row.get("sample_id"))
        )
        duplicates = sorted([value for value, count in counts.items() if count > 1])
    numeric_stats = {
        col: _numeric_stats([row.get(col, "") for row in rows])
        for col in columns
        if inferred[col] == "number"
    }
...
def tool_quality_check(ctx: ToolContext, args: dict[str, object]) -> ToolResult:
    ...
    metadata_path = (
        ctx.path_resolver(str(args["metadata_path"]))
        if args.get("metadata_path")
        else experiment_dir / "metadata.csv"
    )
    spectra_dir = (
        ctx.path_resolver(str(args["spectra_dir"]))
    ...
    if not metadata_path.is_file():
        findings.append(
            _finding(
                ...
                "critical",
                f"metadata file not found: {relpath(ctx, metadata_path)}",
            )
        )
        ...
        findings.append(
            _finding(
                ...
                "critical",
                "metadata must be CSV in this build",
            )
        )
    else:
        metadata_rows = _read_csv_dicts(metadata_path)
        columns = (
            set(metadata_rows[0].keys()) if metadata_rows else set(_read_csv_header(metadata_path))
        )
            ...
            if not sample_id:
                findings.append(
                    _finding(
                        ...
                        "missing_sample_id",
                        "critical",
                        f"metadata row {idx} has empty sample_id",
                        f"row={idx}",
                    )
                )
                continue
            ...
            if count > 1:
                findings.append(
                    _finding(
                        ...
                        "duplicate_sample_id",
                        "critical",
                        f"sample_id appears {count} times",
                        str(count),
                    )
                )
    ...
    if not spectra_dir.is_dir():
        findings.append(
            _finding(
                ...
                "missing_spectra_dir",
                "critical",
                f"spectra directory not found: {relpath(ctx, spectra_dir)}",
            )
        )
        ...
        if not match:
            findings.append(
                _finding(
                    ctx,
                    batch_id,
                    _sample_id_from_name(path),
                    path,
                    "invalid_filename",
                    "warning",
                    "filename should match sample_id_method.csv",
                    path.name,
                )
            )
        ...
        if sample_id not in metadata_id_set:
            findings.append(
                _finding(
                    ...
                    "critical",
                    "spectra file has no matching metadata record",
                    path.name,
                )
            )
        ...
        if sample_id not in file_sample_ids:
            findings.append(
                _finding(
                    ...
                    batch_id,
                    sample_id,
                    spectra_dir / f"{sample_id}_*.csv",
                    "missing_spectra_file",
                    "critical",
                    "metadata sample has no corresponding spectra CSV",
                )
            )
    ...
    return ToolResult(
        ...
        "\n".join(text),
        metadata=metadata,
        affected_paths=[metadata["qc_summary_path"]],
...
def _run_single_preprocess(
    ctx: ToolContext, args: dict[str, object], batch_id: str, script_name: str, script_path: Path
) -> ToolResult:
    input_path = resolve_workspace_path(ctx.root, str(args["input_path"]))
    if not input_path.is_file():
        return ToolResult(
            False, f"input file not found: {relpath(ctx, input_path)}", error_code="not_file"
        )
    ...
    ok = row["status"] == "ok"
    affected = (
        [row["output_path"], relpath(ctx, summary_path)] if ok else [relpath(ctx, summary_path)]
    )
    return ToolResult(
        ok,
...
def _run_batch_preprocess(
    ...
    if not input_dir.is_dir():
        return ToolResult(
            False,
            f"input directory not found: {relpath(ctx, input_dir)}",
            error_code="not_directory",
            metadata={"batch_id": batch_id, "script_name": script_name, "mode": "batch"},
        )
    ...
    skipped_samples = (
        _critical_samples(ctx.root, batch_id) if (skip_critical or only_qc_passed) else set()
    )
    ...
    for input_path in candidates:
        ...
        if sample_id in skipped_samples:
            rows.append(
                _preprocess_row(
                    ctx, script_name, input_path, Path(""), "skipped", "critical QC finding", 0.0
                )
            )
            continue
    ...
    affected = [row["output_path"] for row in rows if row["status"] == "ok"] + [
        relpath(ctx, summary_path)
    ]
    metadata = {
        "batch_id": batch_id,
        "script_name": script_name,
    ...
    if not candidates:
        return ToolResult(
            False,
            "no preprocessing inputs matched",
            metadata=metadata,
            error_code="no_inputs",
            affected_paths=[relpath(ctx, summary_path)],
            workspace_changed=True,
        )
    ok = success > 0
    return ToolResult(
...
def _critical_samples(root: Path, batch_id: str) -> set[str]:
    ...
    return {
        row.get("sample_id", "")
        for row in _read_csv_dicts(qc_path)
        if row.get("severity") == "critical" and row.get("sample_id")
    }


def _run_preprocess_one(
    ctx: ToolContext, script_path: Path, script_name: str, input_path: Path, output_path: Path
) -> dict[str, str]:
    ...
    error = (
        ...
        else (completed.stderr.strip() or f"returncode={completed.returncode}")
    )
...
def tool_summarize_outputs(ctx: ToolContext, args: dict[str, object]) -> ToolResult:
    ...
    if not output_dir.exists():
        raise ToolExecutionError(
            f"output directory not found: outputs/{batch_id}", error_code="not_found"
        )
...
def tool_generate_report(ctx: ToolContext, args: dict[str, object]) -> ToolResult:
    ...
    qc_path = (
        ...
        else ctx.root / "outputs" / batch_id / "qc_summary.csv"
    )
    if not qc_path.is_file():
        return ToolResult(
            False,
            f"QC summary not found: {relpath(ctx, qc_path)}",
            error_code="not_found",
    ...
    preprocessed = (
        sorted((output_dir / "preprocessed").glob("*.csv"))
        if (output_dir / "preprocessed").is_dir()
        else []
    )
    ...
    return ToolResult(
        ...
        metadata={
            ...
            "preprocess_skipped_count": preprocess_skipped,
        },
...
def tool_export_workflow_log(ctx: ToolContext, args: dict[str, object]) -> ToolResult:
    ...
    return ToolResult(
        ...
        metadata={
            "batch_id": batch_id,
            "workflow_log_path": relpath(ctx, trace_path),
            "pending_runtime_export": True,
        },
    ...
    try:
        ...
        return [
            _finding(
                ...
                "critical",
                f"cannot parse spectra CSV: {exc}",
            )
        ]
        ...
        if field not in columns:
            findings.append(
                _finding(
                    ...
                    "missing_spectrum_column",
                    "critical",
                    f"required spectra column is missing: {field}",
                    field,
                )
            )
    if len(rows) < MIN_SPECTRA_POINTS:
        findings.append(
            _finding(
                ...
                "too_few_points",
                "warning",
                f"spectra has too few points: {len(rows)}",
                str(len(rows)),
            )
        )
        ...
        if _is_blank(row.get("intensity")):
            findings.append(
                _finding(
                    ...
                    "missing_intensity",
                    "critical",
                    f"row {idx} has missing intensity",
                    f"row={idx}",
                )
            )
            ...
            findings.append(
                _finding(
                    ...
                    "non_numeric_intensity",
                    "critical",
                    f"row {idx} has non-numeric intensity",
                    f"row={idx}",
                )
            )
        else:
...
__all__ = [
    ...
    "tool_inspect_table",
    "tool_quality_check",
    "tool_run_preprocess_script",
    "tool_summarize_outputs",
    "tool_generate_report",
    "tool_export_workflow_log",
]
